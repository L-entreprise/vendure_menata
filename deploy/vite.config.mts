...
// Signals vendure-config.ts that it's being imported only for plugin discovery,
// so missing runtime secrets resolve to placeholders instead of throwing.
...
export default defineConfig({
    ...
    plugins: [
        vendureDashboardPlugin({
            vendureConfigPath: pathToFileURL('./vendure-config.ts'),
            pathAdapter: {
                // Root output structure at the repo root (parent of deploy/) so the
                // `../plugins/*` files the config imports stay inside the temp dir
                // instead of escaping above it.
                sourceRoot: path.resolve(__dirname, '..'),
                // With that sourceRoot, the compiled config lands under deploy/.
                getCompiledConfigPath: ({ outputPath, configFileName }) =>
                    path.join(outputPath, 'deploy', configFileName),
            },
            theme: {
                light: {
                    background: '#f7f9fe',
